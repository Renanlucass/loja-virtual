...
export default function PedidoConfirmadoPage({ pedido }) {
  ...
  const formaPagamentoDisplay = {
    cartao: 'Cartão',
    boleto: 'Boleto',
    pix: 'PIX',
  }[pedido.forma_pagamento] || pedido.forma_pagamento;

  const parcelas = pedido.parcelas ?? (pedido.forma_pagamento === 'cartao' ? 1 : null);
    ...
    <main className="container mx-auto px-4 py-10 max-w-2xl">
      <div className="bg-white p-6 sm:p-8 rounded-xl shadow-lg border">
        ...
        <div className="space-y-2 text-gray-700 text-left">
          ...
          <div className="flex justify-between font-bold text-lg"><span>Total do Pedido</span><span>{formatPrice(pedido.total_pedido ?? pedido.subtotal)}</span></div>
        </div>

        <div className="border-t my-4"></div>

        <div className="text-left text-gray-700 space-y-1">
          <p><strong>Forma de pagamento:</strong> {formaPagamentoDisplay}</p>

          {pedido.forma_pagamento === 'cartao' && (
            <>
              <p><strong>Tipo do cartão:</strong> {pedido.tipo_cartao === 'credito' ? 'Crédito' : pedido.tipo_cartao === 'debito' ? 'Débito' : 'N/A'}</p>
              
              {pedido.tipo_cartao === 'credito' && (
